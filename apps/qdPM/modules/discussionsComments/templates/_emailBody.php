  ...
  $comments_html .=  
    '<tr>
      <td style="font-family: Arial; font-size: 13px; color: black; border-bottom:1px dashed LightGray; padding:2px;">' . replaceTextToLinks($c->getDescription()) . '
        <div>' . get_component('attachments','attachmentsList',array('bind_type'=>'discussionsComments','bind_id'=>$c->getId())) . '</div>
        <div>' . get_component('discussionsComments','info',array('c'=>$c)) . '</div>            
      </td>
...
$column1 = '<h3>' . link_to($discussions->getProjects()->getName(),'projectsComments/index?projects_id=' . $discussions->getProjectsId(),array('absolute'=>true)) . ': '.  link_to($discussions->getName(),'discussionsComments/index?projects_id=' . $discussions->getProjectsId() . '&discussions_id=' . $discussions->getId(),array('absolute'=>true)) . '</h3><p>' . $comments_html . '</p>';
