...
?>
<form class="form-horizontal" role="form" id="departments" action="<?php echo url_for('departments/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php include_partial('global/formValidator',array('form_id'=>'departments')); ?>
