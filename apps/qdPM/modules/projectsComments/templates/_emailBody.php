  ...
  $comments_html .=  
    '<tr>
      <td style="font-family: Arial; font-size: 13px; color: black; border-bottom:1px dashed LightGray; padding:2px;">' . replaceTextToLinks($c->getDescription()) . '
        <div>' . get_component('attachments','attachmentsList',array('bind_type'=>'projectsComments','bind_id'=>$c->getId())) . '</div>            
      </td>
...
$column1 = '<h3>' . link_to($projects->getName(),'projectsComments/index?projects_id=' . $projects->getId(),array('absolute'=>true)) . '</h3><p>' . $comments_html . '</p>';
