...
?>
  <tr id="related_task_<?php echo $tasks['id'] ?>">
    ...
    <td style="text-align: right;"><?php if(!$is_email) echo '<a href="javascript: removeRelated(\'related_task_' . $tasks['id'] . '\',\'' . url_for('app/removeRelatedTaskWithDiscussions?tasks_id=' . $tasks['id'] . '&discussions_id=' . $sf_request->getParameter('discussions_id')) . '\')" class="btn btn-default btn-xs purple" title="' . __('Delete Related') . '"><i class="fa fa-trash-o"></i></a>' ?></td>
  </tr>  
